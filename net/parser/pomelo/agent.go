...
func NewAgent(app cfacade.IApplication, conn net.Conn, session *cproto.Session) Agent {
	...
	if clog.PrintLevel(zapcore.DebugLevel) {
		clog.Debug("[sid = %s,uid = %d] Agent create. [count = %d, ip = %s]",
			agent.SID(),
			agent.UID(),
			Count(),
			agent.RemoteAddr(),
...
func (a *Agent) readChan() {
	defer func() {
		if clog.PrintLevel(zapcore.DebugLevel) {
			clog.Debug("[sid = %s,uid = %d] Agent read chan exit.",
				a.SID(),
				a.UID(),
			)
		}
...
func (a *Agent) writeChan() {
	...
	defer func() {
		if clog.PrintLevel(zapcore.DebugLevel) {
			clog.Debug("[sid = %s,uid = %d] Agent write chan exit.", a.SID(), a.UID())
		}
		...
		case <-ticker.C:
			{
				...
				if lastAt < deadline {
					if clog.PrintLevel(zapcore.DebugLevel) {
						clog.Debug("[sid = %s,uid = %d] Check heartbeat timeout.", a.SID(), a.UID())
					}
					return
				}
			}
...
func (a *Agent) closeProcess() {
	...
	if err := a.conn.Close(); err != nil {
		clog.Debug("[sid = %s,uid = %d] Agent connect closed. [error = %s]",
			a.SID(),
			a.UID(),
			err,
		)
	...
	if clog.PrintLevel(zapcore.DebugLevel) {
		clog.Debug("[sid = %s,uid = %d] Agent closed. [count = %d, ip = %s]",
			a.SID(),
			a.UID(),
			Count(),
			a.RemoteAddr(),
...
func (a *Agent) processPacket(packet *pomeloPacket.Packet) {
	...
	if !found {
		if clog.PrintLevel(zapcore.DebugLevel) {
			clog.Warn("[sid = %s,uid = %d] Packet type not found, close connect! [packet = %+v]",
				a.SID(),
				a.UID(),
				packet,
			)
...
func (a *Agent) processPending(data *pendingMessage) {
	...
	if err != nil {
		clog.Warn("[sid = %s,uid = %d] Payload marshal error. [data = %s]",
			a.SID(),
			a.UID(),
			data.String(),
		)
...
func (a *Agent) sendPending(typ pomeloMessage.Type, route string, mid uint32, v interface{}, isError bool) {
	...
	if len(a.chPending) >= cmd.writeBacklog {
		clog.Warn("[sid = %s,uid = %d] send buffer exceed. [typ = %v, route = %s, mid = %d, val = %+v, err = %v]",
			a.SID(),
			a.UID(),
			typ,
			route,
...
func (a *Agent) ResponseMID(mid uint32, v interface{}, isError ...bool) {
	...
	if clog.PrintLevel(zapcore.DebugLevel) {
		clog.Debug("[sid = %s,uid = %d] Response ok. [mid = %d, isError = %v]",
			a.SID(),
			a.UID(),
			mid,
			isErr,
...
func (a *Agent) Push(route string, val interface{}) {
	...
	if clog.PrintLevel(zapcore.DebugLevel) {
		clog.Debug("[sid = %s,uid = %d] Push ok. [route = %s]",
			a.SID(),
			a.UID(),
			route,
		)
...
func (a *Agent) Kick(reason interface{}, closed bool) {
	...
	if err != nil {
		clog.Warn("[sid = %s,uid = %d] Kick packet encode error.[reason = %+v, err = %s]",
			a.SID(),
			a.UID(),
			reason,
			err,
	...
	if clog.PrintLevel(zapcore.DebugLevel) {
		clog.Debug("[sid = %s,uid = %d] Kick ok. [reason = %+v, closed = %v]",
			a.SID(),
			a.UID(),
			reason,
			closed,
