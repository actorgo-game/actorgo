...
func (s *Component) Init() {
	...
	if dbIDList.LastError() != nil || dbIDList.Size() < 1 {
		clog.Warn("[nodeID = %s] `db_id_list` property not exists.", s.App().NodeID())
		return
	}
		...
		for i := 0; i < dbGroup.Size(); i++ {
			...
			for _, key := range dbIDList.Keys() {
				...
				if !mysqlConfig.Enable {
					clog.Panic("[dbName = %s] is disabled!", mysqlConfig.DbName)
				}

				db, err := s.createORM(mysqlConfig)
				if err != nil {
					clog.Panic("[dbName = %s] create orm fail. error = %s", mysqlConfig.DbName, err)
				}

				s.ormMap[groupID][mysqlConfig.ID] = db
				clog.Info("[dbGroup =%s, dbName = %s] is connected.", mysqlConfig.GroupID, mysqlConfig.ID)
			}
		}
	}
}
...
func (s *Component) GetHashDb(groupID string, hashFn HashDb) (*gorm.DB, bool) {
	...
	if !found {
		clog.Warn("groupID = %s not found.", groupID)
		return nil, false
	}
