...
func (p *HttpServer) Static(relativePath string, staticDir string) {
	...
	if !ok {
		clog.Error("static dir path not found. staticDir = %s", staticDir)
		return
	}
...
func (p *HttpServer) StaticFile(relativePath, filepath string) {
	...
	if !ok {
		clog.Error("static dir path not found. filePath = %s", filepath)
		return
	}
...
func (p *HttpServer) Stop() {
	...
	clog.Info("shutdown http server on %s", p.server.Addr)
}
