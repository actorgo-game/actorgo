...
func (p *ETCD) Load(app cfacade.IApplication) {
	...
	if clusterConfig.LastError() != nil {
		clog.Fatal("etcd config not found. err = %v", clusterConfig.LastError())
		return
	}
	...
	clog.Info("[etcd] init complete! [endpoints = %v] [leaseID = %d]", p.config.Endpoints, p.leaseID)
}

func (p *ETCD) OnStop() {
	key := fmt.Sprintf(registerKeyFormat, p.app.NodeID())
	_, err := p.cli.Delete(context.Background(), key)
	clog.Info("etcd stopping! err = %v", err)

	err = p.cli.Close()
	if err != nil {
		clog.Warn("etcd stopping error! err = %v", err)
	}
}
...
func (p *ETCD) init() {
	...
	if err != nil {
		clog.Fatal("etcd connect fail. err = %v", err)
		return
	}
