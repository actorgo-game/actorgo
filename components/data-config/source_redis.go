...
func (r *SourceRedis) Init(_ IDataConfig) {
	...
	if dataConfig.Unmarshal(&r.redisConfig) != nil {
		clog.Warn("[data_config]->[%s] node in `%s` file not found.", r.Name(), cprofile.Name())
		return
	}
...
func (r *SourceRedis) newRedis() {
	r.rdb = redis.NewClient(&redis.Options{
		...
		OnConnect: func(ctx context.Context, cn *redis.Conn) error {
			clog.Info("data config for redis connected")
			return nil
		},
	})
}
...
func (r *SourceRedis) newSubscribe() {
	...
	for {
		...
		case ch := <-sub.Channel():
			...
			clog.Info("[name = %s] trigger file change.", ch.Payload)

			data, err := r.ReadBytes(ch.Payload)
			if err != nil {
				clog.Warn("[name = %s] read data error = %s", ch.Payload, err)
				continue
			}
...
func (r *SourceRedis) Stop() {
	clog.Info("close redis client [address = %s]", r.Address)
	r.close <- true
