...
func (f *SourceFile) Init(_ IDataConfig) {
	...
	if err != nil {
		clog.Panic("Unmarshal fileConfig fail. err = %v", err)
		return
	}
	...
	if err != nil {
		clog.Panic("AddFilterHook extName fail. err = %v", err)
		return
	}
	...
	if err != nil {
		clog.Panic("[name = %s] join path fail. err = %v.", f.Name(), err)
		return
	}

	err = f.watcher.Add(f.monitorPath)
	if err != nil {
		clog.Panic("New watcher error. path=%s, err = %v", f.monitorPath, err)
		return
	}
			...
			case ev := <-f.watcher.Event:
				{
					...
					clog.Info("Trigger file change. [name = %s]", configName)
...
func (f *SourceFile) Stop() {
	...
	clog.Info("Remote watcher [path = %s, err = %v]", f.monitorPath, err)
