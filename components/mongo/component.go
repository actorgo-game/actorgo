...
func (s *Component) Init() {
	...
	if mongoIDList.LastError() != nil || mongoIDList.Size() < 1 {
		clog.Warn("[nodeID = %s] `mongo_id_list` property not exists.", s.App().NodeID())
		return
	}
	...
	for _, groupID := range mongoConfig.Keys() {
		...
		for i := 0; i < dbGroup.Size(); i++ {
			...
			for _, key := range mongoIDList.Keys() {
				...
				clog.Info("[dbGroup =%s, dbName = %s] is connected.", groupID, id)
			}
		}
	}
}
...
func CreateDatabase(uri, dbName string, timeout ...time.Duration) (*mongo.Database, error) {
	...
	if err != nil {
		clog.Warn("Ping DB is error. [dbName = %s, host = %v, err = %v]", dbName, opts.Hosts, err)
		return nil, err
	}

	clog.Info("Ping DB is ok. [dbName = %s, host = %v] ", dbName, opts.Hosts)
